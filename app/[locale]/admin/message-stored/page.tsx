...
export default function MessageStoredPage() {
  ...
  useEffect(() => {
    if (selectedConversationId) {
      fetchScheduledMessages(selectedConversationId);
    }
    ...
    const pollConversation = async () => {
      try {
        ...
        if (hasNewMessage || hasOptimistic) {
          ...
          // Use requestAnimationFrame to wait for DOM to update first
          if (wasAtBottom) {
  ...
  const fetchScheduledMessages = async (convId: string) => {
    try {
  ...
  const cancelScheduledMessage = async (id: string) => {
    ...
    try {
      ...
      setScheduledMessages((prev) => prev.filter((s) => s._id !== id));
    } catch (e) {
      alert(`Failed to cancel scheduled message: ${e}`);
    }
  };

  const saveScheduledEdit = async () => {
    if (!editingScheduled || !editScheduledDateTime) return;
    ...
    try {
      const res = await fetch("/api/messages/schedule", {
        ...
        body: JSON.stringify({
          ...
          message: editingScheduledMessage,
        }),
      });
      const data = await res.json();
      ...
      setScheduledMessages((prev) =>
        prev.map((s) =>
          s._id === editingScheduled._id
            ? {
                ...s,
                scheduledAt: new Date(editScheduledDateTime).toISOString(),
                message: editingScheduledMessage || s.message,
              }
            : s,
        ),
      );
                                  ...
                                  <div className="flex items-center gap-1.5 text-gray-400">
                                    ...
                                    <span className="text-xs text-gray-400">
                                      ...
                                      {(msg as any).callDuration
                                        ? ` · ${Math.floor(((msg as any).callDuration as number) / 60)}:${String(((msg as any).callDuration as number) % 60).padStart(2, "0")}`
                                        : ""}
                                    </span>
                                  </div>
                                  <div className="flex-1 h-px bg-gray-200" />
