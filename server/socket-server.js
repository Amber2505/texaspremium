...
async function checkAndImportCsvEmail() {
  try {
    console.log('📧 Checking for AgencyMatrix CSV email...');
    ...
    // Search for AgencyMatrix email from today or yesterday
    ...
    for await (const msg of client.fetch(
      {
        since: since,
        subject: 'AgencyMatrix - Transaction Detail CSV File',
      },
    ...
    if (messages.length === 0) {
      console.log('📧 No AgencyMatrix CSV email found yet');
      await client.logout();
      return { found: false };
    }
    ...
    console.log(`📧 Found AgencyMatrix email: ${latest.envelope.subject}`);

    // Download and parse the full message
    const download = await client.download(latest.uid, undefined, { uid: true });
    const parsed = await simpleParser(download.content);

    // Find the CSV attachment
    const csvAttachment = parsed.attachments?.find(
      (a) => a.filename?.toLowerCase().endsWith('.csv') || a.contentType === 'text/csv'
    );

    if (!csvAttachment) {
      console.log('📧 No CSV attachment found in email');
      await client.logout();
      return { found: false, error: 'No CSV attachment' };
    }

    console.log(`📧 Found CSV attachment: ${csvAttachment.filename} (${csvAttachment.size} bytes)`);

    // Send to the accounting import API
    const csvText = csvAttachment.content.toString('utf-8');
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.texaspremiumins.com';

    const res = await fetch(`${appUrl}/api/accounting/import-raw`, {
      method: 'POST',
    ...
    if (data.success) {
      // Verify the CSV date matches today CST before accepting
      const cstToday = new Date().toLocaleDateString('en-CA', { timeZone: 'America/Chicago' }).slice(0, 7); // "2026-06"
      if (data.monthKey && data.monthKey !== cstToday) {
        console.warn(`⚠️ CSV date mismatch — email contains ${data.monthKey} but today is ${cstToday}. Skipping import.`);
        await client.logout();
        return { found: true, skipped: true, reason: 'date_mismatch' };
      }
      console.log(`✅ Auto CSV import complete: ${data.inserted} inserted, ${data.deleted} replaced`);
      lastCsvImportDate = todayDate;
    } else {
      console.error('❌ Auto CSV import failed:', data.error);
    }

    await client.logout();
    return { found: true, inserted: data.inserted };

  } catch (err) {
    console.error('❌ CSV email check error:', err.message);
    return { found: false, error: err.message };
  }
}

// Catch-up: scan last N days of emails and import any missing CSVs
async function catchUpCsvImports(daysBack = 7) {
  console.log(`📧 Running CSV catch-up scan for last ${daysBack} days...`);
  const results = [];
  ...
  try {
    ...
    const messages = [];
    for await (const msg of client.fetch(
      { since, subject: 'AgencyMatrix - Transaction Detail CSV File' },
      { envelope: true, uid: true }
    )) {
      messages.push(msg);
    }

    console.log(`📧 Found ${messages.length} AgencyMatrix email(s) in last ${daysBack} days`);

    // Connect to MongoDB to check what's already imported
    const mongoDb = mongoClient.db('db');
    const accountingCol = mongoDb.collection('accounting_info');

    for (const msg of messages) {
      try {
        const download = await client.download(msg.uid, undefined, { uid: true });
        ...
        if (!csvAttachment) {
          console.log(`📧 No CSV in: ${msg.envelope.subject}`);
          continue;
        }

        const csvText = csvAttachment.content.toString('utf-8');

        // Peek at the first data row to get the dateKey
        const lines = csvText.split('\n').filter(l => l.trim());
        if (lines.length < 2) continue;

        // Parse first data row to get date
        const firstDataLine = lines[1];
        const dateMatch = firstDataLine.match(/^(\d{2})-(\d{2})-(\d{2,4})/);
        if (!dateMatch) continue;
        const year = dateMatch[3].length === 2 ? '20' + dateMatch[3] : dateMatch[3];
        const monthKey = `${year}-${dateMatch[1]}`;

        // Check if this month already has data in MongoDB
        const existingCount = await accountingCol.countDocuments({
          dateKey: { $regex: `^${monthKey}` }
        });

        if (existingCount > 0) {
          console.log(`⏭️ Skipping ${monthKey} — already has ${existingCount} records in DB`);
          results.push({ monthKey, status: 'skipped', reason: 'already_imported', count: existingCount });
          continue;
        }

        // Not in DB — import it
        console.log(`📥 Importing missing CSV for ${monthKey}...`);
        const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://www.texaspremiumins.com';
        const res = await fetch(`${appUrl}/api/accounting/import-raw`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (data.success) {
          console.log(`✅ Catch-up import complete for ${monthKey}: ${data.inserted} records`);
          results.push({ monthKey, status: 'imported', inserted: data.inserted });
        } else {
          console.error(`❌ Catch-up import failed for ${monthKey}:`, data.error);
          results.push({ monthKey, status: 'failed', error: data.error });
        }

        // Small delay between imports to avoid hammering the API
        await new Promise(r => setTimeout(r, 1000));

      } catch (msgErr) {
        console.error(`❌ Error processing email:`, msgErr.message);
        results.push({ status: 'error', error: msgErr.message });
      }
    }

    await client.logout();
    console.log(`📧 Catch-up complete. Results:`, results);
...
function scheduleCsvEmailImport() {
  setInterval(async () => {
    const now = new Date();
    const cstTime = new Date(now.toLocaleString('en-US', { timeZone: 'America/Chicago' }));
    const hour = cstTime.getHours();
    ...
    // Run between 1:30 AM and 2:00 AM CST — email arrives by then
    if (hour === 1 && minute >= 30 && lastCsvImportDate !== todayDate) {
      console.log('📧 1:30 AM CST — Checking for AgencyMatrix CSV email...');
      const result = await checkAndImportCsvEmail();
      if (result.found && !result.skipped) {
        lastCsvImportDate = todayDate;
        console.log('✅ Daily CSV auto-import complete');
      } else if (result.skipped) {
        console.warn(`⚠️ Import skipped: ${result.reason}`);
      }
      // If not found yet, it will retry every 30s until 2 AM
    }

    // Also try at 2:00 AM as a fallback if 1:30 didn't find it
    if (hour === 2 && minute >= 0 && minute < 5 && lastCsvImportDate !== todayDate) {
      console.log('📧 2 AM fallback — Retrying AgencyMatrix CSV check...');
      const result = await checkAndImportCsvEmail();
      if (result.found && !result.skipped) lastCsvImportDate = todayDate;
    }

    // Reset at midnight
    if (hour === 0 && minute === 0 && lastCsvImportDate !== null) {
      lastCsvImportDate = null;
    }
  }, 30000);

  console.log('⏰ AgencyMatrix CSV auto-import scheduled for 1:30 AM CST daily');
}
...
app.post('/catch-up-csv', async (req, res) => {
  try {
    const daysBack = parseInt(req.body?.daysBack || '7', 10);
    console.log(`📧 Manual catch-up triggered for last ${daysBack} days`);
    const result = await catchUpCsvImports(daysBack);
    res.json(result);
  } catch (err) {
